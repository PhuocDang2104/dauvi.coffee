...
export function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  const startingPrice = Math.min(...(availableVariants.length ? availableVariants : product.variants).map((variant) => variant.price.amount));

  return (
    <article className="product-card group flex h-full flex-col overflow-hidden border border-basalt-900/10 bg-white/80 transition duration-300 hover:border-forest-800/25">
      <div className="product-card-image relative aspect-square overflow-hidden bg-paper-100">
        <Link
      ...
      <div className="flex flex-1 flex-col p-5 md:p-6">
        ...
        <h3 className="mt-2 font-display text-[1.55rem] font-semibold leading-tight tracking-[-0.035em]">
          <Link className="hover:text-forest-600" href={`/shop/${product.slug}`}>
            {product.displayName}
          </Link>
        </h3>
        ...
        <div className="mt-auto flex items-end justify-between gap-4 pt-6">
          <div>
            ...
            {!availableVariants.length ? <p className="text-xs text-roast-700">Tạm hết hàng</p> : null}
          </div>
