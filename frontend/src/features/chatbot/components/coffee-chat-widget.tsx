...
import { ArrowUpRight, Coffee, Send, X } from "lucide-react";
  ...
  return (
    <Dialog.Root modal={false} open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button
          type="button"
          className={`group fixed right-3 z-[60] grid size-14 place-items-center rounded-full border border-honey-500/35 bg-forest-950 text-honey-500 shadow-lg transition duration-300 hover:-translate-y-1 hover:bg-forest-800 focus-visible:ring-2 focus-visible:ring-honey-500 focus-visible:ring-offset-2 ${pathname.startsWith("/shop/") ? "bottom-[10.5rem] md:bottom-[5.6rem]" : "bottom-[5.6rem]"} lg:bottom-6 lg:right-6 ${pathname === "/" && !hasClearedHero ? "max-lg:pointer-events-none max-lg:translate-y-3 max-lg:opacity-0" : ""}`}
          aria-label="Mở trợ lý cà phê DẤU VỊ"
        >
          <Coffee aria-hidden="true" className="relative size-6 lg:size-7" strokeWidth={1.8} />
          <span className="absolute -right-1 -top-1 rounded-full bg-paper-100 px-1.5 text-[10px] font-bold text-forest-950">AI</span>
        </button>
      </Dialog.Trigger>
        ...
        >
          <div className="relative overflow-hidden bg-forest-950 px-5 py-5 text-white">
            ...
            <div className="relative flex items-center gap-3 pr-10">
              <span className="grid size-11 place-items-center rounded-full border border-honey-500/40 text-honey-500"><Coffee aria-hidden="true" size={20} /></span>
              <div>
          ...
          <div ref={conversationRef} role="log" aria-label="Cuộc trò chuyện" className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain bg-paper-100/55 p-4" aria-live="polite" aria-relevant="additions text">
          ...
          <div className="border-t border-basalt-900/10 bg-mist-50 p-3">
            <div className="mb-2 flex gap-1.5 overflow-x-auto pb-1">
              {QUICK_PROMPTS.map((prompt) => (
                <button key={prompt} type="button" disabled={pending} onClick={() => void sendMessage(prompt)} className="min-h-11 shrink-0 rounded-full border border-forest-950/12 bg-white px-3 text-xs font-bold text-forest-950 transition hover:border-honey-500/50 hover:bg-paper-100 disabled:opacity-50">
                  {prompt}
                </button>
              ))}
            </div>
